      ...
      if (!window.zappyContactFormLoaded) {
        try {
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '73d51b25-5223-4824-b88b-089c3465038c',
              name: name,
              email: email,
              subject: company ? `Contact Form - ${company}` : 'Contact Form Submission',
              message: message,
              phone: phone || null
            })
          });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Joca Media משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
